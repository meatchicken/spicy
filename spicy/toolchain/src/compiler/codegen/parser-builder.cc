...
// Helper function to get any of the specified unit properties.
std::optional<type::unit::item::Property> anyProperty(const type::Unit& unit,
                                                      const std::vector<const char*>& properties) {
    for ( const auto& property : properties ) {
        if ( auto x = unit.propertyItem(property) )
            return x;
    }
...
struct ProductionVisitor
    ...
    void skipRegExp(const hilti::ctor::RegExp& c) {
        // Start a new block to avoid name clashes.
        auto block = builder()->addBlock();
        pushBuilder(block);

        builder()->addLocal(ID("__re"), builder::regexp(c.value(), AttributeSet({Attribute("&anchor")})));

        builder()->addLocal(ID("ncur"), state().cur);
        auto ms = builder::local("ms", builder::memberCall(builder::id("__re"), "token_matcher", {}));
        ...
        pushBuilder(match);
        builder()->addAssign(state().cur, builder::id("ncur"));
        builder()->addBreak();
        popBuilder();
    ...
    void operator()(const production::Unit& p) {
        ...
        if ( const auto& skipPre = anyProperty(p.unitType(), {"%skip-pre", "%skip"}) ) {
            auto ctor = skipPre->expression()->as<hilti::expression::Ctor>().ctor();
            if ( auto c = ctor.tryAs<hilti::ctor::RegExp>() )
                skipRegExp(*c);
        }
        ...
        if ( const auto& skipPost = anyProperty(p.unitType(), {"%skip-post", "%skip"}) ) {
            auto ctor = skipPost->expression()->as<hilti::expression::Ctor>().ctor();
            if ( auto c = ctor.tryAs<hilti::ctor::RegExp>() )
                skipRegExp(*c);
        }
